...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (wallpaperSource) {
        wallpaperSource.addEventListener('change', () => {
            if (wallpaperSource.value === 'local') {
                ...
                if (wallpaperUnsplashConfig) wallpaperUnsplashConfig.style.display = 'none';
            } else if (wallpaperSource.value === 'unsplash') {
                if (wallpaperLocalConfig) wallpaperLocalConfig.style.display = 'none';
                if (wallpaperUnsplashConfig) wallpaperUnsplashConfig.style.display = 'block';
            } else {
                if (wallpaperLocalConfig) wallpaperLocalConfig.style.display = 'none';
                if (wallpaperUnsplashConfig) wallpaperUnsplashConfig.style.display = 'none';
            }
    ...
    function toggleColumnCountDisplay(mode) {
        if (columnCountContainer) {
            columnCountContainer.style.display = mode === 'columns' ? 'block' : 'none';
        }
    }
    ...
    function loadSettings() {
        ...
        chrome.storage.sync.get(['extensionSettings'], function (result) {
            ...
            if (wallpaperFolderPath) wallpaperFolderPath.value = settings.wallpaperFolderPath || '';
            if (wallpaperFrequency) wallpaperFrequency.value = settings.wallpaperFrequency || 1;
            ...
            if (filterOpacity) {
                filterOpacity.value = settings.filterOpacity !== undefined ? settings.filterOpacity : 0.3;
            ...
            if (iconGap) {
                iconGap.value = settings.iconGap !== undefined ? settings.iconGap : (settings.iconSpacing || 8);
                updateIconGapDisplay(iconGap.value);
            }
            if (categoryGap) {
                categoryGap.value = settings.categoryGap !== undefined ? settings.categoryGap : 12;
            ...
            if (iconBorderRadius) {
                iconBorderRadius.value = settings.iconBorderRadius !== undefined ? settings.iconBorderRadius : 8;
                updateBorderRadiusDisplay(iconBorderRadius.value);
            }
            if (iconBorderColor) iconBorderColor.value = ensureFullHex(settings.iconBorderColor);
            if (iconBgColor) iconBgColor.value = ensureFullHex(settings.iconBgColor);
            if (iconBgOpacity) {
                iconBgOpacity.value = settings.iconBgOpacity !== undefined ? settings.iconBgOpacity : 0.75;
                if (iconBgOpacityValue) iconBgOpacityValue.textContent = Number(iconBgOpacity.value).toFixed(2);
            }
            if (bookmarkFontFamily) bookmarkFontFamily.value = settings.bookmarkFontFamily || 'sans-serif';
            if (bookmarkFontSize) {
                bookmarkFontSize.value = settings.bookmarkFontSize || 12;
                updateBookmarkFontSizeDisplay(bookmarkFontSize.value);
            }
            if (bookmarkFontColor) bookmarkFontColor.value = ensureFullHex(settings.bookmarkFontColor);
            ...
            if (sectionBgColor) sectionBgColor.value = ensureFullHex(settings.sectionBgColor) || '#43587a';
            if (sectionBgOpacity) {
                sectionBgOpacity.value = settings.sectionBgOpacity !== undefined ? settings.sectionBgOpacity : 0.55;
                if (sectionBgOpacityValue) sectionBgOpacityValue.textContent = Number(sectionBgOpacity.value).toFixed(2);
            }
            if (sectionLineColor) sectionLineColor.value = ensureFullHex(settings.sectionLineColor) || '#7d3af8';

            if (nameDisplay) nameDisplay.value = settings.nameDisplay || 'always';
            if (textBehavior) textBehavior.value = settings.textBehavior || 'truncate';
            ...
            if (columnCount && settings.columnCount) {
                ...
                if (columnCountValue) columnCountValue.textContent = settings.columnCount;
            }

            if (sidebarWidth) {
                sidebarWidth.value = settings.sidebarWidth || 240;
                if (sidebarWidthValue) sidebarWidthValue.textContent = (settings.sidebarWidth || 240) + 'px';
            }
    ...
    function saveSettings() {
        ...
        saveTimeout = setTimeout(() => {
            const settings = {
                wallpaperFolderPath: wallpaperFolderPath ? wallpaperFolderPath.value : '',
                wallpaperFrequency: wallpaperFrequency ? parseFloat(wallpaperFrequency.value) : 1,
                ...
                bookmarkMinWidth: bookmarkMinWidth ? parseInt(bookmarkMinWidth.value) : 150,
                iconBorderRadius: iconBorderRadius ? parseInt(iconBorderRadius.value) : 8,
                ...
                bookmarkFontFamily: bookmarkFontFamily ? bookmarkFontFamily.value : 'sans-serif',
                bookmarkFontSize: bookmarkFontSize ? parseInt(bookmarkFontSize.value) : 12,
                bookmarkFontColor: bookmarkFontColor ? bookmarkFontColor.value : '#e2e8f0',
                ...
                quickLinksSize: quickLinksSize ? parseInt(quickLinksSize.value || 13) : 13,
                sectionPadding: sectionPadding ? parseInt(sectionPadding.value || 15) : 15,
                sectionBgColor: sectionBgColor ? sectionBgColor.value : '#43587a',
                sectionBgOpacity: sectionBgOpacity ? parseFloat(sectionBgOpacity.value) : 0.55,
    ...
    function updateOpacityDisplay(value) {
        if (filterOpacityValue) filterOpacityValue.textContent = `${Math.round(value * 100)}%`;
    }
