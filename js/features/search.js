...
export class SearchManager {
                ...
                chrome.bookmarks.getSubTree(root.id, nodes => {
                    ...
                    if (rootNode.children) {
                        rootNode.children.forEach(spaceNode => {
                            if (!spaceNode.url) { // Space folder
                                const titleParts = spaceNode.title.match(/^(\p{Emoji_Presentation}|\p{Emoji}\uFE0F?)\s*(.+)$/u);
                                let spaceIcon = '📁';
                                ...
                                if (spaceNode.children) {
                                    spaceNode.children.forEach(groupNode => {
                                        if (!groupNode.url && groupNode.children) { // Group folder
                                            groupNode.children.forEach(bookmarkNode => {
                                                if (bookmarkNode.url) { // Bookmark link
                                                    bookmarks.push({
                                                        id: bookmarkNode.id,
    ...
    setSpaceFilter(spaceId) {
        ...
        buttons.forEach((btn, idx) => {
            ...
            } else if (spaceId !== 'all' && idx > 0 && this.spaces[idx - 1].id === spaceId) {
                btn.classList.add('active');
            } else {
    ...
    performSearch() {
        ...
        if (query) {
            this.filteredBookmarks = list.filter(bm => 
    ...
    renderNoBookmarksMessage() {
        this.resultsList.innerHTML = '';
        this.noResults.textContent = 'Nenhum espaço ou bookmark configurado no momento.';
        this.noResults.style.display = 'block';
    }
}
