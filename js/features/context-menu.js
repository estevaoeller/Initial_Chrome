...
export class ContextMenuManager {
    ...
    this.menu.querySelectorAll('.context-menu-item').forEach((item) => {
      item.addEventListener('click', (e) => {
        ...
        if (action && action !== 'move') {
          // 'move' action is handled by submenu
          e.stopPropagation();
  ...
  renderSubmenu() {
    ...
    const groupsToMoveTo = this.currentBookmarksList.filter(
      (g) => g.id !== currentCategoryId,
    );
  ...
  moveBookmarkToGroup(targetCategory) {
    ...
    if (bookmarkId && targetCategoryId) {
      chrome.bookmarks.move(bookmarkId, { parentId: targetCategoryId }, () => {
        ...
        const sourceIndex = this.currentCategory.links.findIndex(
          (l) => l.id === bookmarkId,
        );
        if (sourceIndex > -1) {
    ...
    switch (action) {
      ...
      case 'open-incognito':
        chrome.windows.create(
          { url: this.currentLink.url, incognito: true },
          () => {
            if (chrome.runtime.lastError) {
              console.warn(
              ...
              alert(
                "Não foi possível abrir em modo anônimo. Certifique-se de que a opção 'Permitir em modo anônimo' está ativa nas configurações desta extensão (chrome://extensions).",
              );
            }
          },
        );
      ...
      case 'copy':
        navigator.clipboard
          .writeText(this.currentLink.url)
          .then(() => {
