...
export class SidebarManager {
    ...
    init() {
        ...
        if (this.closeSpaceModalBtn) {
            this.closeSpaceModalBtn.addEventListener('click', () => this.closeModal());
        }
        ...
        if (this.deleteSpaceBtn) {
            this.deleteSpaceBtn.addEventListener('click', () => this.handleDeleteSpace());
        }
        ...
        if (this.spaceModal) {
            this.spaceModal.addEventListener('keydown', (e) => {
                if (e.key === 'Tab' && this.spaceModal.classList.contains('show')) {
                    const focusables = Array.from(this.spaceModal.querySelectorAll('button, input, [href], select, textarea, [tabindex]:not([tabindex="-1"])')).filter(el => {
    ...
    openModal(spaceId = null) {
        ...
        if (spaceId) {
            ...
            if (space) {
                if (this.spaceModalTitle) this.spaceModalTitle.textContent = 'Editar Espaço';
            ...
            // Create Mode
            this.editingSpaceId = null;
            if (this.spaceModalTitle) this.spaceModalTitle.textContent = 'Novo Espaço';
    ...
    handleSaveSpace() {
        ...
        const icon = (this.spaceIconInput && this.spaceIconInput.value.trim()) || '📁';

        if (this.editingSpaceId) {
            // Edit
            renameSpace(this.editingSpaceId, name, icon, () => {
                const space = this.currentSpaces.find(s => s.id === this.editingSpaceId);
                if (space) {
                    space.name = name;
                    space.icon = icon;
                }
    ...
    handleDeleteSpace(spaceId = null) {
        ...
        if (!confirm('Deseja realmente excluir este espaço e TODOS os seus marcadores? Esta ação não pode ser desfeita.')) return;

        deleteSpace(idToDelete, () => {
            this.currentSpaces = this.currentSpaces.filter(s => s.id !== idToDelete);
            this.renderSpacesList(this.currentSpaces);
