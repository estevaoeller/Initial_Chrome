...
export class SettingsQuickLinksManager {
  ...
  renderQuickLinksUI(links) {
    ...
    if (links.length === 0) {
      this.quickLinksList.innerHTML =
        '<div style="padding:15px; text-align:center; opacity:0.7;">Nenhum link adicionado</div>';
      return;
    }
    links.forEach((link, index) => {
      ...
      item.style.cssText =
        'display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:1px solid var(--settings-border); background:var(--input-bg); margin-bottom:5px; border-radius:4px; transition: transform 0.2s, box-shadow 0.2s;';
      ...
      item.innerHTML = `
                ...
                <button class="delete-link-btn" data-index="${index}" title="Excluir link" aria-label="Excluir link" style="background:transparent; border:none; cursor:pointer; padding:5px; color:var(--danger, #ef4444); display:flex; align-items:center;"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events:none;"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg></button>
            `;
  ...
  handleDragEnd(e) {
    ...
    if (this.quickLinksList) {
      this.quickLinksList
        .querySelectorAll('.draggable-link')
        .forEach((item) => {
