export class ShortcutsManager {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.altKey && e.key.toLowerCase() === 'g') {
                ...
                if (newGroupBtn) {
                    ...
                    alert('Por favor, certifique-se de que os favoritos estão carregados para criar um grupo.');
                }
                return;
            }
    ...
    addBookmarkFlow() {
        ...
        if (!groups || groups.length === 0) {
            alert('Por favor, crie um grupo primeiro utilizando Alt+G antes de adicionar um bookmark.');
            return;
        }
        ...
        const groupChoice = prompt(`Escolha o grupo pelo número (ou digite o nome do grupo):\n${groupList}`, '1');
        ...
        if (!isNaN(choiceIdx) && choiceIdx >= 0 && choiceIdx < groups.length) {
            ...
            selectedGroup = groups.find(g => g.name.toLowerCase() === trimmedChoice) || groups[0];
        }

        import('../modules.js').then(m => {
            m.addBookmarkToChrome(activeSpaceId, selectedGroup.name, { name, url }, (createdNode) => {
