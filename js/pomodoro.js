...
export async function initPomodoro() {
  ...
  if (toggleBtn) {
    ...
    toggleBtn.addEventListener('click', (e) => {
      e.preventDefault();
      pomodoroWidget.style.display =
        pomodoroWidget.style.display === 'none' ? 'flex' : 'none';
    });
  }
  ...
  if (localState.pomodoroActive && localState.pomodoroEndTime) {
    ...
    restoreTimer(
      localState.pomodoroEndTime,
      localState.pomodoroMode,
      localState.pomodoroTogglEntryId,
    );
  } else {
...
async function startTimer() {
  // Determine duration
  const localState = await new Promise((resolve) =>
    chrome.storage.local.get(['pomodoroMode'], resolve),
  );
  const mode = localState.pomodoroMode || 'work';

  const minutes =
    mode === 'work'
      ? currentSettings.pomodoroWork || 25
      : currentSettings.pomodoroBreak || 5;
  ...
  if (mode === 'work' && togglWorkspaceId && currentSettings.togglApiToken) {
    ...
    const res = await startTogglTimer(
      currentSettings.togglApiToken,
      togglWorkspaceId,
      desc,
      projId,
    );
...
async function stopTimer() {
  ...
  const localState = await new Promise((resolve) =>
    chrome.storage.local.get(['pomodoroTogglEntryId', 'pomodoroMode'], resolve),
  );
  if (
    localState.pomodoroTogglEntryId &&
    currentSettings.togglApiToken &&
  ...
  const nextMode =
    (localState.pomodoroMode || 'work') === 'work' ? 'break' : 'work';
