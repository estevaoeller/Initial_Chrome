...
export async function getTogglProjects(token, workspaceId) {
  if (!token || !workspaceId)
    return { success: false, error: 'Token ou WorkspaceID ausente' };
...
export async function startTogglTimer(
  ...
  if (!token || !workspaceId)
    return { success: false, error: 'Autenticação ou Workspace ausente' };
  ...
  try {
    ...
    const response = await fetch(
      `${BASE_URL}/workspaces/${workspaceId}/time_entries`,
      {
        method: 'POST',
        headers: getAuthHeader(token),
...
export async function stopTogglTimer(token, workspaceId, timeEntryId) {
  if (!token || !workspaceId || !timeEntryId)
    return { success: false, error: 'Dados insuficientes para parar timer' };

  try {
    const response = await fetch(
      `${BASE_URL}/workspaces/${workspaceId}/time_entries/${timeEntryId}/stop`,
      {
        method: 'PATCH',
        headers: getAuthHeader(token),
      },
